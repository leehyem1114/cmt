...
const MaterialManager = (function() {
    ...
    async function init() {
        try {
            console.log('원자재 기준정보 관리 초기화를 시작합니다.');
            ...
            console.log('원자재 기준정보 관리 초기화가 완료되었습니다.');
        } catch (error) {
            console.error('초기화 중 오류 발생:', error);
            await AlertUtil.showError('초기화 오류', '원자재 기준정보 관리 초기화 중 오류가 발생했습니다.');
        }
    }
    ...
    async function initGrid() {
        try {
            ...
            // DOM 요소 존재 확인
            const gridElement = document.getElementById('materialGrid');
            if (!gridElement) {
                throw new Error('materialGrid 요소를 찾을 수 없습니다. HTML을 확인해주세요.');
            }

            // 서버에서 받은 데이터 활용
            const gridData = window.materialList || [];
    ...
    async function appendRow() {
        try {
            ...
            // 그리드에 새 행 추가
            await GridUtil.addNewRow('materialGrid', newRowData, {
                at: 0,
                focus: true
            });
        } catch (error) {
    ...
    async function searchData() {
        try {
            const keyword = document.getElementById('materialInput').value;
    ...
    async function saveData() {
        try {
            ...
            const grid = GridUtil.getGrid('materialGrid');
            if (!grid) {
                throw new Error('materialGrid를 찾을 수 없습니다.');
            }
            ...
            // 변경된 데이터 추출
            const changes = await GridUtil.extractChangedData('materialGrid');
    ...
    async function deleteRows() {
        try {
            ...
            const result = await GridUtil.deleteSelectedRows('materialGrid', {
                confirmTitle: "삭제 확인",
                confirmMessage: "선택한 원자재 정보를 삭제하시겠습니까?",
                ...
                onAfterDelete: async () => {
                    ...
                    try {
                        ...
                        await AlertUtil.notifyDeleteError('삭제 실패', '원자재 정보 삭제 중 API 오류가 발생했습니다.');
                    }
                }
            });
    ...
    async function saveSearchCondition() {
        try {
            const searchCondition = document.getElementById('materialInput')?.value || '';

            localStorage.setItem('materialSearchCondition', searchCondition);
    ...
    async function loadSearchCondition() {
        try {
            const savedCondition = localStorage.getItem('materialSearchCondition');
            ...
            // 검색 조건 설정
            const searchInput = document.getElementById('materialInput');
            if (searchInput) {
...
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        // await MaterialManager.loadSearchCondition();
    } catch (error) {
        console.error('초기화 중 오류 발생:', error);
        if (window.AlertUtil) {
            await AlertUtil.showError('초기화 오류', '원자재 기준정보 관리 초기화 중 오류가 발생했습니다.');
        } else {
            alert('원자재 기준정보 관리 초기화 중 오류가 발생했습니다.');
        }
    }
});