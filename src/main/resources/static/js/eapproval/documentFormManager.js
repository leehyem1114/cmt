...
const DocumentFormManager = (function() {
    ...
    /**
     * 응답 객체에서 양식 내용을 추출하는 함수
     * @param {Object} data - API 응답 데이터
     * @returns {string|null} - 추출된 양식 내용 또는 null
     */
    function extractFormContent(data) {
        console.log('응답 데이터에서 양식 내용 추출 시작');
        
        // null이나 undefined 확인
        if (!data) {
            console.warn('응답 데이터가 null 또는 undefined입니다.');
            return null;
        }
        
        // 문자열인 경우 그대로 반환
        if (typeof data === 'string') {
            console.log('응답 데이터가 직접 문자열입니다.');
            return data;
        }
        
        // 1단계: 일반적인 필드 이름으로 직접 찾기
        const commonFields = [
        ...
        for (const field of commonFields) {
            if (data[field] !== undefined && data[field] !== null) {
                console.log(`일반 필드에서 내용 발견: ${field}`);
                return data[field];
            }
        }
        
        // 2단계: 'data' 또는 유사한 중간 객체 확인
        const wrapperFields = ['data', 'DATA', 'result', 'RESULT', 'form', 'FORM'];
        for (const wrapper of wrapperFields) {
            if (data[wrapper] && typeof data[wrapper] === 'object') {
                console.log(`중간 래퍼 객체 발견: ${wrapper}, 재귀 검색 시작`);
                const innerContent = extractFormContent(data[wrapper]);
                if (innerContent) {
                    return innerContent;
                }
            }
        ...
        // 3단계: 'content' 또는 유사한 단어가 포함된 모든 키 확인
        for (const key in data) {
            if (typeof key === 'string' && 
                ...
                console.log(`유사 내용 필드 발견: ${key}`);
                return data[key];
            }
        }
        
        // 4단계: 객체를 순회하며 문자열 값을 찾기 (최후의 수단)
        for (const key in data) {
            if (typeof data[key] === 'string' && data[key].length > 20 && 
                (data[key].includes('<') || data[key].includes('>'))) {
                // HTML 같은 태그가 포함된 긴 문자열은 내용일 가능성이 높음
                console.log(`HTML 태그가 포함된 문자열 발견: ${key}`);
                return data[key];
            }
        }
