...
const SimpleGridManager = (function() {
	...
	*/
    ...
    const API_URLS = {
        LIST: '/controller/salary/salaryItem',       // 목록 조회 컨트롤러 URL
    ...
    async function init() {
        try {
            ...
            await UIUtil.registerEventListeners({
				'salaryItemDeleteBtn': deleteRows
	...
	*/
    ...
    async function appendRow() {
        try {
            ...
            const newRowData = {
                SAL_ITEM_TYPE: '',                // 코드 (빈 값)
                SAL_ITEM_NAME: '',                // 이름 (빈 값)
                SAL_ITEM_DESC: '',         // 설명 (빈 값)
                SAL_ITEM_IMPORTANCE: 'Y',          // 사용여부 (기본값 'Y')
                SAL_ITEM_APPLY_YEAR: '',
				SAL_ITEM_UPDATE_DATE: '',
				ROW_TYPE: ''           // 정렬 (빈 값)
                // [✓] 확장 지점: 필요한 필드를 추가하세요
            ...
            await GridUtil.addNewRow('salaryItemGrid', newRowData, {
                at: 0,           // 위치 (0: 최상단, -1: 최하단)
   ...
   /* async function saveData() {
        try {
            ...
            const changes = await GridUtil.extractChangedData('salaryItemGrid');
