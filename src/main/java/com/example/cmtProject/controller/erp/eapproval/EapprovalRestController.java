...
public class EapprovalRestController {
    ...
    public ApiResponse<Map<String, Object>> processApproval(
        ...
        try {
            ...
            documentService.processApproval(docId, approverId, decision, comment);
            String message = decision.equals("승인") ? "결재가 승인되었습니다." : "결재가 반려되었습니다.";
    ...
    public ApiResponse<List<DocumentDTO>> getDrafterDocuments(
            @RequestParam Integer drafterId,
            @RequestParam(required = false) String status) {
        
        log.debug("기안 문서 목록 조회 요청: 기안자={}, 상태={}", drafterId, status);
        try {
            List<DocumentDTO> documents;
            if (status != null && !status.isEmpty()) {
                documents = documentService.getDocumentsByDrafterAndStatus(drafterId, status);
            } else {
                documents = documentService.getDrafterDocuments(drafterId);
            }
     ...
     */
    @GetMapping("/pending")
    public ApiResponse<List<DocumentDTO>> getPendingDocuments(@RequestParam Integer approverId) {
        log.debug("결재 대기 문서 목록 조회 요청: 결재자={}", approverId);
        try {
            List<DocumentDTO> documents = documentService.getPendingDocuments(approverId);
            return ApiResponse.success(documents);
        } catch (Exception e) {
    ...
    public ApiResponse<List<EmpListPreviewDTO>> getApprovers() {
        ...
        try {
            ...
            if (employees.isEmpty()) {
                log.warn("직원 목록이 비어 있습니다");
            } else {
                log.debug("직원 목록 {}명 조회됨", employees.size());
            }
            
            // EmpListPreviewDTO 목록을 직접 반환
